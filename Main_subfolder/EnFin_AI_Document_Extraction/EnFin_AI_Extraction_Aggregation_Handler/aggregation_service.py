...
def _get_extraction_table():
    ...
    if _extraction_table is None:
        ...
        if table_name:
            ...
            logger.warning(
                "DYNAMODB_EXTRACTION_TABLE environment variable not set"
            )
...
def get_process_config(process: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if item:
            ...
            logger.info(
                "Loaded process config for '%s' from DynamoDB", process
            )
        else:
        ...
        logger.exception(
            "Failed to fetch config for process '%s'", process
        )
        return None
...
def query_processed_files(
    app_no: str, process: str
) -> List[Dict[str, Any]]:
    """Query all processed files for an app_no and process.

    Uses the ``app_no-index`` GSI on the Result table with ``process``
    as the sort key.
    ...
    try:
        ...
        while True:
            query_kwargs = {
                "IndexName": "app_no-index",
                "KeyConditionExpression": (
                    "app_no = :app_no AND process = :process"
                ),
...
def update_latest_submission_flag(
    path: str, process: str, is_latest: bool
) -> None:
    ...
    try:
        ...
        logger.exception(
            "Failed to update latest_submission for path=%s", path
        )
