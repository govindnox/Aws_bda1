...
class ExtractionCandidate:
    ...
    def sort_key(self):
        ...
        ts = self.submission_timestamp or ""
        return (-self.confidence_score, ts)  # Negate for DESC
...
class ConflictInfo:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "conflictingValues": [
                cv.to_dict() for cv in self.conflicting_values
            ],
        }
...
class AggregationResult:
    ...
    aggregated_fields: Dict[str, Optional[str]] = field(
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "AggregationConfig":
        ...
        return cls(
            ...
            conflict_threshold=float(
                data.get("conflict_threshold", 0.05)
            ),
            submission_window_minutes=int(
                data.get("submission_window_minutes", 30)
            ),
...
class AggregationController:
    ...
    def handle_event(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for record in event.get("Records", []):
            ...
            try:
                ...
                logger.exception(
                    "Failed to process aggregation record %s", message_id
                )
                batch_failures.append({"itemIdentifier": message_id})

        if batch_failures:
            logger.warning(
                "Returning %d batch failures", len(batch_failures)
            )
    ...
    def _process_record(self, record: Dict[str, Any]) -> None:
        ...
        all_files = aggregation_service.query_processed_files(
            app_no, process
        )
        ...
        # 4 — Determine latest submission window
        latest_timestamp, cutoff_timestamp = (
            self._determine_submission_window(
        ...
        if not files_in_window:
            ...
            files_in_window = all_files  # Fallback: use all files

        # 6 — Extract and group entities from files in window
        all_extractions = self._extract_entities_from_files(
            files_in_window
        )

        if not all_extractions:
            logger.info(
                "No extractable entities found for app_no=%s", app_no
            )
            return

        # 7 — Aggregate with confidence-based conflict resolution
        agg_result = self._aggregate_extractions(
            all_extractions, agg_config
        )
        ...
        for entity, candidates in all_extractions.items():
            ...
            if len(candidates) > 1:
                ...
                confidence_diff = (
                    ...
                    - second_candidate.confidence_score
                )
    ...
    def _resolve_program(
        ...
        utility = agg_result.aggregated_fields.get(
            "utility_provider", ""
        )
        ...
        for _key, prog_cfg in agg_config.program_configs.items():
            cfg_states = [
                s.upper().strip()
                for s in prog_cfg.get("states", [])
            ]
            cfg_utilities = [
                u.upper().strip()
                for u in prog_cfg.get("utilities", [])
            ]
    ...
    def _push_aggregated_to_salesforce(
        ...
        """
        sf_enabled = (
            os.environ.get("SF_ENABLED", "false").lower() == "true"
        )
        if not sf_enabled:
        ...
        if not sf_endpoint:
            logger.warning(
                "SF endpoint not configured — skipping aggregation push"
            )
            return
        ...
        for f in files_in_window:
            files_used.append(
                {
                    "path": f.get("path", ""),
                    "submissionTimestamp": f.get(
                        "submission_timestamp", ""
                    ),
                    "isLatestSubmission": f.get(
        ...
        data_payload = {
            ...
            "missingMandatoryFields": (
                agg_result.missing_mandatory_fields
            ),
            "fieldsRequiringReview": (
                agg_result.fields_requiring_review
            ),
        ...
        try:
            ...
            if not access_token:
                logger.warning(
                    "Failed to get SF token — skipping aggregation push"
                )
                return
            ...
            logger.exception(
                "Failed to push aggregated result to Salesforce"
            )
            raise
    ...
    def _get_sf_token(self) -> Optional[str]:
        ...
        if not token_table_name or not sf_username:
            logger.error(
                "SF_TOKEN_TABLE or SF_USERNAME not configured"
            )
            return None
        ...
        try:
            ...
            if "Item" in response:
                ...
                if token and issued_at:
                    diff_time = int(
                        os.environ.get("SF_TOKEN_DIFF_TIME", "300")
                    )
            ...
            return self._fetch_new_sf_token(
                token_table, sf_username
            )
    ...
    def _fetch_new_sf_token(
        self, token_table, sf_username: str
    ) -> Optional[str]:
        ...
        response = requests.post(
            auth_url, data=secret, timeout=30
        )
