...
Modification History:
    2026-04-21 - CR-04: SF error returns dict not string.
                 CR-06: Token schema unified to issued_at/diff_time.
                 CR-14: f-string logging replaced with %s style.
"""
...
# Lazy-init AWS clients (CR-28 partial — keeps cold-start lean)
_dynamodb = None
...
def _get_secrets_manager():
    """Lazy-initialise the Secrets Manager client."""
...
def lambda_handler(event: Dict[str, Any], context) -> Dict[str, Any]:
    ...
    """
    # CR-14: Use %s logging style throughout.
    logger.info(
        "Received event with %d records",
    ...
    for record in event.get("Records", []):
        ...
        try:
            ...
            logger.exception("Failed to process record %s: %s", message_id, exc)
...
def process_record(record: Dict[str, Any]) -> None:
    ...
    # CR-14: %s style logging.
    logger.info("Processing SF push: path=%s, process=%s", path, process)
    ...
    table = _get_dynamodb().Table(extraction_table)
    ...
    # Validate state
    if file_record.get("state") not in ("Processed", "Failed"):
        logger.warning(
            "File not in Processed or Failed state: path=%s, state=%s",
            path,
    ...
    try:
        ...
        logger.exception("Failed to parse extraction result: %s", exc)
        extraction_result = {}
    ...
    logger.info("SF push successful: path=%s", path)
...
def push_to_salesforce(
    ...
    # CR-14: %s style logging.
    logger.info("Calling Salesforce API: url=%s", full_url)

    response = requests.post(full_url, headers=headers, json=payload, timeout=30)

    if response.status_code not in [200, 201]:
        error_msg = (
            f"Salesforce API returned {response.status_code}: " f"{response.text}"
        )
        logger.error(error_msg)
        # CR-04: Return a dict — not a raw string — so json.dumps()
        # in update_file_record produces valid structured JSON.
        # Also raise so SQS retries the message.
...
def get_sf_token() -> str:
    ...
    token_table = _get_dynamodb().Table(token_table_name)
...
def fetch_new_sf_token() -> str:
    ...
    secret_response = _get_secrets_manager().get_secret_value(SecretId=secret_name)
...
def update_file_record(
    ...
    Args:
        ...
        sf_response: Salesforce API response dict.
    """
    # CR-19: timezone-aware timestamp.
    table.update_item(
