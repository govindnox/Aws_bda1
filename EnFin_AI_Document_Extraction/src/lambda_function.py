...
def bda_result_handler(event, _context):
    """Entry point for the BDA Result Lambda.

    Triggered by S3 ObjectCreated events when BDA writes result.json
    to the bda-output/ prefix.
    ...
    Returns:
        dict: ``{ "batchItemFailures": [...] }`` for partial-batch support.
    """
    logger.info("Received BDA result event: %s", json.dumps(event))

    try:
        from bda_result_controller import BDAResultController

        controller = BDAResultController()
        return controller.handle_event(event)
    except Exception as e:
        logger.exception("Unhandled error in bda_result_handler: %s", e)
        return {
            "statusCode": 500,
            "body": json.dumps({"error_message": str(e)}),
        }