...
class BDAPipeline:
    ...
    def run(
        ...
        # Environment-scoped prefix — one bucket for all envs, so each env
        # lives under its own top-level folder
        output_prefix = f"{config.environment}/bda-output/{app_no}/{self._config.process}/{encoded_path}"
        ...
        invocation_arn = bda_service.invoke_async(
            ...
            input_key=path,
            output_bucket=config.bda.output_bucket,
