...
class ExtractionPipeline:
    ...
    def run(
        ...
        if isinstance(fields_data, list):
            ...
            raw_fields = {
                field["name"]: field for field in fields_data if "name" in field
            }
        else:
    ...
    def _merge_responses(responses: List[LLMResponse]) -> LLMResponse:
        ...
        if isinstance(base_fields, list):
            ...
            merged_fields = {
                field["name"]: field for field in base_fields if "name" in field
            }
        else:
            # Already a dict
        ...
        for resp in responses[1:]:
            ...
            if isinstance(resp_fields, list):
                resp_fields_dict = {
                    field["name"]: field for field in resp_fields if "name" in field
                }
            else:
